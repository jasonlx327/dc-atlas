...
export default function Home({ initialPayload = null, view = "home" }: { initialPayload?: Partial<AtlasPayload> | null; view?: SiteView }) {
  ...
  if (view === "home") return <main id="top" className="compact-home zh-console">
    ...
    <section className="section home-industry-preview" id="industry"><div className="section-title inverse"><div><span className="section-no">02</span><p>INFRASTRUCTURE MAP</p><h2>从算力到需求</h2></div><p>连接算力硬件、网络、液冷、电力、园区与模型需求的最新公开信号。</p></div><div className="industry-gateway-grid">{chainStages.map((stage) => { const item = payload?.chainNews?.[stage.key]?.[0]; return <a href={`/industry?tab=chain#${stage.key}`} key={stage.key}><span>{stage.no}</span><i><ChainIcon type={stage.key} /></i><div><strong>{stage.title}</strong><small>{stage.caption}</small>{item && <p>{item.title}</p>}</div><ArrowIcon /></a>; })}</div><a className="section-more-link" href="/industry">进入完整产业中心 <ArrowIcon /></a></section>
