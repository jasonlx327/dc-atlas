...
type Story = { id: string; title: string; summary?: string; publishedAt?: string | null; permalink?: string; sourceUrl?: string | null; sourceName?: string | null; category?: string | null; region?: string; milestone?: string; scale?: string; whyItMatters?: string; listedTicker?: string };
...
const chainMeta: Record<ChainKey, { no: string; title: string; detail: string }> = {
  ...
  cooling: { no: "03", title: "Liquid cooling", detail: "CDUs, cold plates and thermal systems" },
  power: { no: "04", title: "Power & energy", detail: "Grid access, substations and generation" },
  campus: { no: "05", title: "Data-center campuses", detail: "Land, construction and commissioned MW" },
  model: { no: "06", title: "Models & cloud demand", detail: "The demand layer for infrastructure" },
};
...
export default function EnglishHome({ initialPayload = null }: { initialPayload?: Partial<AtlasPayload> | null }) {
  ...
  const dailyItems = useMemo(() => payload?.aiDaily ? [payload.aiDaily.lead, ...payload.aiDaily.sections.flatMap((section) => section.items), ...payload.aiDaily.flashes].filter((item): item is Story => Boolean(item)).slice(0, 6) : [], [payload]);
  const chainGroups = Object.entries(payload?.chainNews ?? {}).map(([stage, items]) => [stage, items.filter((item) => item.region !== "中国").slice(0, 2)] as const);
  const eventList = payload?.upcomingEvents ?? [];
  const pulse = (payload?.idcPulse ?? []).filter((item) => item.region !== "中国");
  const listed = (payload?.listedCompanyNews ?? []).filter((item) => !/\.(SH|SZ)\b/.test(item.listedTicker ?? ""));
  const nvidiaProducts = payload?.nvidiaProducts ?? [];
  const nvidiaNews = payload?.nvidiaNews ?? [];
  const silicon = (payload?.chinaChipNews ?? []).slice(0, 3);
  const supernodes = (payload?.supernodes ?? []).slice(0, 1);
  ...
  return <main className="english-home-page english-full-page" lang="en">
    ...
    <section className="english-hero english-full-hero"><p>GLOBAL DATA CENTER INTELLIGENCE · SOURCE FIRST</p><h1>AI infrastructure<br /><em>through an investor lens.</em></h1><p>IDC Atlas follows the U.S. and global infrastructure cycle through verified project disclosures, listed-company updates, hyperscaler CAPEX, power, cooling and market signals. A selective China Watch retains only developments with clear competitive or supply-chain relevance.</p><div><a href="#pulse">Read the latest pulse</a><a href="#calendar">US earnings &amp; CAPEX watch</a></div><dl><div><dt>GLOBAL PROJECT PULSE</dt><dd>{pulse.length || "—"}</dd><small>verified non-China signals</small></div><div><dt>US LISTED COMPANIES</dt><dd>{listed.length || "—"}</dd><small>source-linked disclosures</small></div><div><dt>LAST UPDATE</dt><dd>{payload ? "LIVE" : loadError ? "OFFLINE" : "…"}</dd><small>{payload ? formatDate(payload.generatedAt) : "connecting"}</small></div></dl></section>

    <section className="section pulse-section english-full-section english-pulse-lead" id="pulse"><SectionHead copy={["01 · GLOBAL & U.S. INFRASTRUCTURE PULSE", "Latest infrastructure pulse", "The most recent verified projects and listed-company disclosures, filtered for global and U.S. readers."]} /><div className="english-full-subhead"><strong>Verified projects</strong><span>United States and global · last 45 days</span></div><div className="english-story-grid">{pulse.slice(0, 6).map((item) => <StoryCard key={item.id} item={item} eyebrow="VERIFIED PROJECT" />)}</div><div className="english-full-subhead"><strong>US listed-company watch</strong><span>{listed.length} source-linked disclosures</span></div><div className="english-story-grid">{listed.slice(0, 3).map((item) => <StoryCard key={item.id} item={item} eyebrow="US LISTED COMPANY" />)}</div></section>
